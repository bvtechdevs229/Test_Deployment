...
const isActive = (history, path) => {
  ...
  return {color: '#0293d6'} //Home button
  else
  return {color: '#FFFFFF'} //Letters of the navigation
}
const isPartActive = (history, path) => {
  if (history.location.pathname.includes(path))
  return {color: '#8ceb34'} //My Stores button
  else
    return {color: '#ffffff'}
}
  ...
  <AppBar position="static">
    <Toolbar>
      ...
      <div>
        ...
        <Link to="/shops/all">
          <Button style={isActive(history, "/shops/all")}>Tech Stores</Button>
        </Link>
        <Link to="/cart">
